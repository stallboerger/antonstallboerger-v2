---
title: Essentry Design system
description: A extensive design system, offering a consistent set of guidelines, components, and patterns.
image: essentry-mockup-cover.jpg
year: 2024-01-31

links: 
  - title: essentry.com
    url: https://essentry.com
---

The essentry design system is a comprehensive design system that offers a consistent set of guidelines, components, and patterns. It is designed to help teams build products faster and more efficiently by providing a shared language and a set of reusable components.

<figure>
    <Image 
        src={`/images/projects/essentry-design-system/essentry-components.jpg`}
        alt={`A overview of components of the Essentry Design System`}
        width={3200}
        height={2400}
        priority
    />
</figure>

## Guides

The design system includes guidelines for typography, color, spacing, and other design elements to ensure a consistent look and feel across all products.

<figure>
    <Image 
        src={`/images/projects/essentry-design-system/essentry-guides.jpg`}
        alt={`Three examples of guidelines of the Essentry Design System`}
        width={3200}
        height={2400}
    />
</figure>